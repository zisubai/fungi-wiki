...
备份默认写入不纳入 Git 的 `backups/` 目录。脚本会在完成后调用 `pg_restore --list` 验证归档可读；也可通过 `POSTGRES_CONTAINER`、`POSTGRES_DB`、`POSTGRES_USER` 和 `BACKUP_DIR` 覆盖默认值。恢复属于覆盖性操作，应在独立数据库完成恢复演练后再用于现有环境。
