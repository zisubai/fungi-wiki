...
- 文献来源、实验结论、证据等级和评分维护
- 功能专题库及各专题已发布菌种数量
- 行业应用案例、问题、方案、结果、成熟度和来源维护
- 菌种主数据及别名、功能、培养条件、证据、应用案例变更版本快照
- 草稿、待审核、已发布、已归档状态管理
- 提交审核、审核通过和驳回流程
- 按关键词、功能、温度、pH、安全等级和来源环境联合筛选
- 基于 PostgreSQL trigram 的拼写容错搜索与相关度排序
- 基于真实 embedding 的语义召回及关键词、向量、质量、运营规则混合排序
- 登录用户菌种收藏和搜索历史
- 运营端同义词、召回加权规则和向量索引管理
- 标准化搜索评测集及 Recall@K、MRR 自动评测
...
Ubuntu 生产服务器可安装仓库提供的用户级 systemd timer。模板默认仓库位于 `~/fungi-wiki`，每天 02:15 后随机延迟最多 15 分钟执行，关机错过后会在下次启动补跑：

```bash
mkdir -p ~/.config/systemd/user
cp deploy/systemd/fungi-wiki-backup.* ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now fungi-wiki-backup.timer
systemctl --user list-timers fungi-wiki-backup.timer
```

如需在用户退出登录后继续运行，应由服务器管理员执行 `sudo loginctl enable-linger "$USER"`。可用 `systemctl --user start fungi-wiki-backup.service` 手工试跑，并通过 `journalctl --user -u fungi-wiki-backup.service` 查看结果。
...
运营端“应用与版本”页面用于维护菌种的行业应用案例。菌种主数据、别名、功能关联、培养条件、文献证据或应用案例发生变化时，数据库会自动保存包含全部关联数据的版本快照，便于追踪数据资产演进。用户端功能专题库按功能标签汇总已发布菌种，并可一键进入对应专题筛选。
...
`GET /api/search` 是用户端默认检索入口。它同时执行关键词/拼写召回、同义词扩展、embedding 余弦语义召回、结构化条件过滤和运营规则加权，默认权重为关键词 45%、向量 40%、数据质量 10%，并叠加命中的召回规则。响应会返回 `semanticEnabled`、`expandedTerms`、`hybridScore` 和匹配原因。

语义检索使用 OpenAI 兼容的 embeddings 协议：
...
也可以连接实现相同协议的腾讯云或本地模型服务。配置后，在运营端“搜索配置”执行“重建向量索引”。若服务暂时不可用，搜索会自动降级到关键词、同义词和规则召回，不中断基础搜索。

用户可注册/登录并通过 `/api/me/favorites` 和 `/api/me/search-history` 跨设备保存菌种收藏及搜索历史。运营端可维护同义词和按查询模式、功能标签、安全等级加权的召回规则。

标准评测集位于 `apps/api/testdata/search-evaluation.json`，运行：

```bash
npm run eval:search
# 发布环境要求语义链路启用：
REQUIRE_SEMANTIC=1 API_URL=https://example.com npm run eval:search
```

评测命令输出逐条排名、Recall@5 和 MRR；默认 Recall@5 低于 0.8 时失败。
