...
export type CombinationItem = {members:{id:string;slug:string;latinName:string;chineseName:string;safetyLevel:string;functionTags:string[];evidenceCount:number}[];score:number;temperatureMin?:number;temperatureMax?:number;phMin?:number;phMax?:number;compatible:boolean;reasons:string[];warning?:string};
